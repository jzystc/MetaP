...
class Trainer:
    def __init__(self, data_loaders, dataset, parameter):
        ...
        print(
            "the number of parameters of pattern learner:",
            sum(param.numel() for param in self.metaP.pattern_learner.parameters()),
        )
        print(
            "the number of parameters of pattern matcher:",
            sum(param.numel() for param in self.metaP.pattern_matcher.parameters()),
        )
        print(
            "the number of parameters of MetaP:",
            sum(param.numel() for name,param in self.metaP.named_parameters()),
        )
        for name,param in self.metaP.named_parameters():
        ...
        if parameter["step"] == "train":
            self.writer = SummaryWriter(
                os.path.join(parameter["log_dir"] + "_", parameter["prefix"])
            )
        # dir
    ...
    def eval_by_relation(self, istest=False, epoch=None):
        ...
        for rel in data_loader.all_rels:
            ...
            t = 0
            ranks = []
            plt_util = PlotUtil(rel.split(":")[-1], dir_suffix="conv")
            while True:
            ...
            print(
                "mean_min: {:.2f}, mean_max: {:.2f}, mean_score: {:.2f}, mean_rank: {:.2f}, mean_num_candidates: {:.2f}".format(
                    np.mean(mean_min),
                    np.mean(mean_max),
                    np.mean(mean_score),
                    np.mean(mean_rank),
