...
class AttentionDistanceNetwork(nn.Module):
    def __init__(self):
        super(AttentionDistanceNetwork, self).__init__()
        # self.W_support = nn.Linear(100, 100)
        # self.W_query = nn.Linear(100, 100)

    def forward(self, query, support, pos_num=0):
        """
        MetaR
        """
        ...
        # score=torch.matmul(query,support.transpose(1,2))
        # query = self.W_support(torch.matmul(attn, support)) + self.W_query( query)
        # support = torch.mean(support, dim=1, keepdim=True)
        # support = support.expand(-1, query.shape[1], -1)
        # score = -torch.norm(query - support, 2, -1)
        if pos_num == 0:
            return score
        else:
...
class MetaP(nn.Module):
    def __init__(self, dataset, parameter):
        ...
        # self.num_support_negative = parameter["num_support_negative"]
        ...
        self.dim = parameter["embed_dim"]
        self.attn_network = AttentionDistanceNetwork()
    ...
    def forward(self, task, iseval=False, curr_rel="", select=False, use_conv=False):
        ...
        # print(pos_relation.shape,support.shape)
        support = self.concat_relation(support, pos_relation)
        # neg_relation = self.get_relation(support_negative, mean=True)
        ...
        if self.aggregator == "attn":
            qry_spt_pos_score = self.attn_network(qry, spt_pos)
            qry_spt_neg_score = self.attn_network(qry, spt_neg)
        else:
